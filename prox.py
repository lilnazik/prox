...
async def https_handler(reader: StreamReader, writer: StreamWriter, request: RawHTTPParser, data: bytes = b''):
    remote_reader, remote_writer = await asyncio.open_connection(request.host, request.port or (80 if data else 443))
    ...
    with closing(remote_writer):
        ...
        print(f"HTTP{'' if data else 'S'} connection established")
